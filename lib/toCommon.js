define([
	...
	return function(inputJson, templates, cssContent) {
		...
		if (lang.isArray(inputJson.type)) {
			locType = "{ ";
			array.forEach(inputJson.type, function(item) {
		...
		function generate(input, path, rows) {
			...
			function checkIfPrimitive(typeArray) {
				return array.every(typeArray, function(item) {
			...
			if (input.required) {
				array.forEach(input.required, function(reqPropName) {
					this.properties[reqPropName] && (this.properties[reqPropName].required = true);
				}, input);
			}

			for (name in input) {
				if (typeof input[name] == "object" && !~array.indexOf(schemaProps, name)) {
					type = input && input[name].type ? input[name].type : ""; // get type
					required = input && input[name].required;
					nullable = (type === "null" || array.indexOf(type, "null") != -1);
					type = typeof type === "object" && lang.isArray(type) && checkIfPrimitive(type) ? type.join() : type; // this is only printable version
					...
					if (name !== "items" && name !== "properties") {
						...
						if (lang.isArray(type)) {
							...
							array.forEach(type, function(item) {
